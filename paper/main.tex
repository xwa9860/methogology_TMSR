...
The group-averaged, component-wise neutronic constants, used in the diffusion equations, are generated with the Monte Carlo neutron transport code Serpent \cite{Serpent2015} using the nuclear data library ENDF/B-VII.0. 
In order to perform transient analysis, these cross-sections are paramatrized as function of reactor operating conditions via a process called computer experiments, in which one runs a Monte Carlo model with a sample of operating conditions and fits a function between the resulting cross-sections and the input data.
...
Unlike liquid salt, the solid fuel elements do not significantly change shape or density in the considered temperature range. 
The change in fuel cross-sections are mainly due to the Doppler broadening of the microscopic cross-sections of the fuel material and thus exhibit a linear-log function of temperature.  It is important for FHRs to compute the temperature distribution inside TRISO particles and fuel pebbles, and use the temperature profile, instead of an average fuel temperature, to compute cross-sections, because during fast transients such as reactivity induced accidents, the heat does not have time to propagate through the fuel pebbles to reach the coolant and therefore the core relies on the temperature reactivity feedback inside the TRISO particles to stabilize.
...
In a porous media model, the convective heat transfer between the fuel pebbles and the coolant is computed from Wakao correlation (Equation \ref{eq:wakao}) \cite{Wakao1979}.
...
In order to determine the temperature distribution inside fuel pebbles and TRISO particles, which is important to compute cross-sections in the fuel region during transient scenarios, we use the 1-D heat diffusion equation in spherical coordinate to model the heat conduction inside solid fuel elements:
...
After a change of variable $U(r,t)=rT(r,t)$, the equation is transformed to the following form:
...
As shown in Figure \ref{fig:model_geom}, the multiphysics reactor core model is composed by three regions: an upper porous region that represents a fuel pebble and flibe salt porous mixture, a lower region with only flibe salt and an outer graphite reflector region. 
...
A vacuum boundary condition is applied at the outer surfaces of the reflector and a symmetry boundary condition is used at the core center-line in 2-D axi-symmetrical models for neutron diffusion. Inlet coolant flow condition and temperature are imposed at the inlet boundary.  This enables modeling of overcooling transient by setting the inlet coolant temperature as a time dependent function. Zero pressure is set to the outlet surfaces. Slip conditions neglect viscous effects are applied to the walls that contain the fluid.  
...
\section{Conclusion}
Strict safety requirement for next-generation reactors along with the progress in computer technology prompts the development, qualification and application of time-dependent multiphysics models for nuclear reactors. This paper presents a methodology for building coupled neutronics/thermal-hydraulics model for FHRs based on multigroup neutron diffusion and porous media CFD, with a multiscale treatment. The method provides high resolution two dimensional or three dimensional results within a reasonable amount of time on a computer with 16.0 GB RAM and four Intel Core equipped with i7-4790K CPU at 4.00 GHz.
...
Although the methodology can be used for all pebble bed FHRs, the results discussed in this paper are based on a specific FHR design, the TMSR SF-1.
